...
export function LeaveDataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
    ...
    return (
        <div>
            <div className="md:flex items-center py-4 justify-between md:space-y-0 space-y-3">
                <p className="text-3xl">Leaves</p>
                <Input
                    placeholder="Filter by Applied By..."
                    value={(table.getColumn("appliedBy")?.getFilterValue() as string) ?? ""}
                    onChange={(event) =>
                        table.getColumn("appliedBy")?.setFilterValue(event.target.value)
                    }
                    className="max-w-sm"
                />
            </div>
